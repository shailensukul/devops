...
source "azure-arm" "autogenerated_moodle" {
  azure_tags = {
    dept = "Engineering"
  ...
  image_sku                         = "22_04-lts"
  location                          = "East US"
  managed_image_name                = "moodleImage"
  managed_image_resource_group_name = "moodleRG"
  os_type                           = "Linux"
...
build {
  sources = ["source.azure-arm.autogenerated_moodle"]

  provisioner "shell" {
    execute_command = "chmod +x {{ .Path }}; {{ .Vars }} sudo -E sh '{{ .Path }}'"
    script = "./moodle/install.sh"
